...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!("A frog has hopped and has {} energy left",self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!("The frog will go to sleep since it has run out of energy");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!("Default value that was generated is a frog with settings {:?}",frog);
        frog
    }
}
